...
1. **טסטים** — אין. `_internals` ב-`assistant.js` כבר חשוף לבדיקות, אבל אין test runner.
   להתחיל מ-`node:test` על המנוע וההרשאות.
2. **תלות ב-instance יחיד** — טיימרים ברקע (`server.js`) והצעות העוזר (`assistant.js`)
   חיים בזיכרון התהליך. סקייל מעל instance אחד ישבור אותם — לתעד/להגן לפני סקייל.
3. **JWT בלי ביטול** — טוקן 30 יום, logout רק מנקה קוקי בצד לקוח. אופציונלי: עמודת `ver`
   שקופצת בשינוי סיסמה.
4. **CSRF** — נשען על `sameSite:lax`. חוסם רוב הבקשות החוצה; token או `sameSite:strict` יחזק.
